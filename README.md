...
新搜索目录名表示 `<r0>-<rm>-<r1>_<w0><wm><w1>`，例如 `1-1-1_424`；目录内正式文件名仍使用 `<r0-rm-r1>`。旧目录 `<r0-rm-r1>` 仍可由离线概率测试读取。每个标准结果目录包含下列同构文件：
...
| `probability_results/<case>/` | `probtest_from_results.py` 的离线概率实验目录，case 保留输入结果目录的旧/新名称 |
